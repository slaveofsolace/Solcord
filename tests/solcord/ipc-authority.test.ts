...
import {isTrustedSolcordIpcUrl, SolcordTimelineIpcAuthority} from "../../src/electron/main/modules/solcord-ipc-authority";


const ACCOUNT_A = "111222333";
const ACCOUNT_B = "444555666";

describe("Solcord private IPC sender origin", () => {
    test("accepts only the exact HTTPS Discord application hosts", () => {
        ...
        ]) expect(isTrustedSolcordIpcUrl(value)).toBeTrue();
        ...
        for (const value of [
            ...
            "not-a-url"
        ]) expect(isTrustedSolcordIpcUrl(value)).toBeFalse();
    });
});

describe("Solcord private IPC capability authority", () => {
    test("rejects a malicious first bind when Timeline is disabled or not bootstrapped", () => {
        ...
        const afterReload = new SolcordTimelineIpcAuthority();
