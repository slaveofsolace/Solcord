...
const GENERATOR_SOURCE = path.join(REPOSITORY_ROOT, "scripts", "build-solcord-runtime-catalog.cjs");
...
function runFixture(acceptance: AcceptanceLedger, candidateDisposition = "SAFE_TO_RUNTIME_TEST", dependencyDisposition = "SAFE_TO_RUNTIME_TEST", options: FixtureOptions = {}): {addons: GeneratedRecord[]; dependencies: GeneratedRecord[];} {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "solcord-acceptance-generator-"));
    ...
    fs.copyFileSync(GENERATOR_SOURCE, path.join(scripts, "build-solcord-runtime-catalog.cjs"));
    ...
    fs.writeFileSync(path.join(catalog, "solcord-reviewed-addons.json"), JSON.stringify(review));
    fs.writeFileSync(path.join(catalog, "solcord-catalog.json"), JSON.stringify({schemaVersion: 1, snapshot: {pluginCount: 1, themeCount: 0}, candidates: []}));
    fs.writeFileSync(path.join(catalog, "solcord-runtime-acceptance.json"), JSON.stringify(acceptance));
    fs.writeFileSync(path.join(catalog, "solcord-security-dispositions.json"), JSON.stringify({
    ...
    const result = Bun.spawnSync({
        cmd: [process.execPath, path.join(scripts, "build-solcord-runtime-catalog.cjs")],
        cwd: root,
        stdout: "pipe",
        stderr: "pipe"
    });
    if (result.exitCode !== 0) throw new Error(`Fixture generator failed: ${result.stderr.toString()}`);
    const generated = fs.readFileSync(path.join(root, "src", "common", "solcord", "addon-catalog.generated.ts"), "utf8");
    return {
        addons: extractGeneratedArray(generated, "SOLCORD_RUNTIME_ADDONS"),
        dependencies: extractGeneratedArray(generated, "SOLCORD_RUNTIME_DEPENDENCIES")
    };
}
...
afterEach(() => {
    for (const root of temporaryRoots.splice(0)) {
        ...
        if (!resolved.startsWith(`${temporary}${path.sep}`) || !path.basename(resolved).startsWith("solcord-acceptance-generator-")) throw new Error("Refusing unsafe acceptance-fixture cleanup.");
...
describe("hash-bound runtime acceptance generator", () => {
    ...
    test("binds every record to the exact suite artifact, commit, and Discord version", () => {
        for (const mutate of [
            ...
            (ledger: AcceptanceLedger) => {ledger.candidates.TestAddon.solcordCommit = "e".repeat(40);},
