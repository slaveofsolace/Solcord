...
import {isSoulCordTransactionOwnedAcceptedArtifact} from "../../src/betterdiscord/modules/soulcord/updater-ownership";
...
function fixture(added: boolean): {pluginRoot: string; transactionRoot: string; transactionId: string; reviewedSha256: string;} {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "soulcord-updater-ownership-"));
    roots.push(root);
    const betterDiscordRoot = path.join(root, "BetterDiscord");
    const pluginRoot = path.join(betterDiscordRoot, "plugins");
    const transactionRoot = path.join(betterDiscordRoot, "soulcord-transactions-v1");
...
function owned(pluginRoot: string, reviewedSha256: string, accepted = true): boolean {
    return isSoulCordTransactionOwnedAcceptedArtifact({accepted, addonFolder: pluginRoot, fileName: FILE_NAME, kind: "plugin", reviewedSha256});
}
...
afterEach(() => {
    for (const root of roots.splice(0)) {
        ...
        if (!resolved.startsWith(`${temporary}${path.sep}`) || !path.basename(resolved).startsWith("soulcord-updater-ownership-")) throw new Error("Refusing unsafe updater fixture cleanup.");
        fs.rmSync(resolved, {recursive: true});
    }
});

describe("SoulCord updater ownership", () => {
    test("holds only an accepted exact file that SoulCord added", () => {
