...
describe("SoulCord Friend Watch domain", () => {
    ...
    test("plans bounded local notices without replaying hydration or exposing identifiers", () => {
        ...
        expect(planSoulCordFriendWatchNotices("daily", [], [...events, reconciliation], {}, now).messages).toEqual([]);
        const daily = planSoulCordFriendWatchNotices("daily", events.slice(0, 1), [...events, reconciliation], {}, now);
        expect(daily.messages).toEqual(["Friend Watch: 7 relationship changes observed today. Open People for the private history."]);
        expect(planSoulCordFriendWatchNotices("daily", events.slice(1, 2), events, daily.state, now).messages).toEqual([]);
        expect(planSoulCordFriendWatchNotices("off", events, events, {}, now).messages).toEqual([]);

        const perEvent = planSoulCordFriendWatchNotices("per-event", events, events, {}, now).messages;
        ...
        expect(pruneSoulCordRelationshipEvents([reconciliation], 30, now)).toEqual([reconciliation]);
    });

    test("uses unknown causality for an uncorrelated friendship disappearance", () => {
        const before = normalizeDiscordRelationships({1: 1});
        const events = reconcileSoulCordRelationships(before, new Map(), [], 10_000, () => "event-1");
    ...
    test("labels removal or block as owner-caused only inside the bounded correlation window", () => {
        ...
        const none = new Map();
        expect(reconcileSoulCordRelationships(friend, none, [{subjectId: "1", action: "remove", observedAt: 8_000}], 10_000, () => "a")[0]).toEqual(expect.objectContaining({label: "Removed by you", confidence: "confirmed"}));
        expect(reconcileSoulCordRelationships(friend, none, [{subjectId: "1", action: "remove", observedAt: 1_000}], 10_000, () => "b")[0]).toEqual(expect.objectContaining({label: "Relationship ended - cause unavailable", confidence: "unknown"}));
    });
    ...
    test("deduplicates and enforces retention and event caps", () => {
        ...
        expect(pruneSoulCordRelationshipEvents([old, fresh], 30, now).map(event => event.eventId)).toEqual(["fresh"]);
        const journal = new SoulCordFriendWatchJournal();
