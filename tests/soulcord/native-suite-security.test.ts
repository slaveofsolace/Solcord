...
import {
    ...
    SOULCORD_TRANSLATION_RESPONSE_MAX_BYTES,
    SoulCordNativeSuiteController
} from "../../src/betterdiscord/modules/soulcord/native-suite";
import {SoulCordDisposalScope} from "../../src/betterdiscord/modules/soulcord/disposal";
import {SOULCORD_VOICE_NOTE_MAX_BYTES, SOULCORD_VOICE_NOTE_MAX_DURATION_MS} from "../../src/common/soulcord/v2-feature-models";
...
const controllers: Array<{controller: SoulCordNativeSuiteController; scope: SoulCordDisposalScope;}> = [];
...
function startController(): SoulCordNativeSuiteController {
    const scope = new SoulCordDisposalScope();
    const controller = new SoulCordNativeSuiteController(scope, {VoiceMessages: true}, {});
    controller.start();
...
describe("SoulCord native-suite security boundaries", () => {
    test("consumes translation JSON through a hard streamed-byte limit", async () => {
        ...
        const oversized = new ReadableStream<Uint8Array>({
            start(controller) {
                controller.enqueue(new Uint8Array(SOULCORD_TRANSLATION_RESPONSE_MAX_BYTES + 1));
                controller.close();
            }
        });
        await expect(readBoundedTranslationJson(new Response(oversized, {headers: {"content-length": "1"}}))).rejects.toThrow("one MiB limit");
        await expect(readBoundedTranslationJson(new Response("{}", {headers: {"content-length": String(SOULCORD_TRANSLATION_RESPONSE_MAX_BYTES + 1)}}))).rejects.toThrow("one MiB limit");
    });
    ...
    test("aborts and releases capture at the byte and duration boundaries", async () => {
        ...
        TestMediaRecorder.instances.at(-1)!.emitData({size: SOULCORD_VOICE_NOTE_MAX_BYTES + 1} as Blob);
        ...
        Object.defineProperty(globalThis, "setTimeout", {
            ...
            value: ((callback: TimerHandler, delay?: number) => {
                if (delay === SOULCORD_VOICE_NOTE_MAX_DURATION_MS) {
