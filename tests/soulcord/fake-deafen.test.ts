...
import {
    applySoulCordFakeDeafenConsentTransition,
    DISCORD_VOICE_STATE_UPDATE_OPCODE,
    normalizeVoiceStatePayload,
    SoulCordFakeDeafenController,
    type SoulCordFakeDeafenDependencies,
    type SoulCordGatewaySocket,
    type SoulCordVoiceStatePayload
} from "../../src/betterdiscord/modules/soulcord/fake-deafen";
...
function harness() {
    ...
    let socket: SoulCordGatewaySocket = {send: originalSend};
    const dependencies: SoulCordFakeDeafenDependencies = {
        getSocket: () => socket,
    ...
    return {
        ...
        set socket(value: SoulCordGatewaySocket) {socket = value;},
...
const deafenedPayload = (): SoulCordVoiceStatePayload => ({guild_id: "456", channel_id: "123", self_mute: false, self_deaf: true});

describe("SoulCord Fake Deafen", () => {
    ...
    test("arms only after a real deafened update, restores local audio, and resynchronizes on disarm", () => {
        const state = harness();
        const controller = new SoulCordFakeDeafenController(state.dependencies);
        ...
        expect((state.sent.at(-1)?.payload as SoulCordVoiceStatePayload).self_deaf).toBeTrue();
        ...
        expect((state.sent.at(-1)?.payload as SoulCordVoiceStatePayload).self_deaf).toBeFalse();
