...
import {
    assertSoulCordBuildAllowed,
    assertSoulCordBuildStillCurrent,
    assertSoulCordPackagingAllowed,
    captureSoulCordBuildProvenance,
    createSoulCordPostBuildManifest,
    readSoulCordBuildProvenance,
    sha256File,
    writeSoulCordBuildProvenance,
    writeSoulCordPostBuildManifest
} from "../../scripts/helpers/build-provenance";

const MODULES = ["soulcord", "main", "preload", "earlyRenderer", "editorPreload", "editor", "editorHtml"];
...
function fixture(): Fixture {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "soulcord-provenance-test-"));
    roots.push(root);
    fs.mkdirSync(path.join(root, "scripts"), {recursive: true});
    fs.writeFileSync(path.join(root, "package.json"), "{\"name\":\"soulcord-test\"}\n");
    ...
    git(root, "config", "user.email", "soulcord-test@example.invalid");
    git(root, "config", "user.name", "SoulCord Test");
    git(root, "add", ".");
...
function capture(
    ...
    return captureSoulCordBuildProvenance(testFixture.root, {
        version: "1.0.0-test",
        mode,
        modules: MODULES,
        ...timing,
...
afterEach(() => {
    for (const root of roots.splice(0)) {
        ...
        if (!resolved.startsWith(`${path.resolve(os.tmpdir())}${path.sep}`) || !path.basename(resolved).startsWith("soulcord-provenance-test-")) {
...
describe("SoulCord build provenance", () => {
    test("labels dirty diagnostics with a deterministic content digest and exposes no local paths", () => {
        ...
        expect(JSON.stringify(first)).not.toContain("soulcord-provenance-test-");
    });
    ...
    test("produces identical metadata, manifests, and ASAR bytes from identical clean inputs", async () => {
        ...
        const firstAsar = path.join(firstOutput, "soulcord.asar");
        const secondAsar = path.join(secondOutput, "soulcord.asar");
        ...
        const checksumText = `${sha256File(firstAsar)}  soulcord.asar\n`;
        fs.writeFileSync(firstChecksums, checksumText);
        fs.writeFileSync(secondChecksums, checksumText);
        const firstManifest = createSoulCordPostBuildManifest(first, {
            asar: firstAsar,
        ...
        const secondManifest = createSoulCordPostBuildManifest(second, {
            asar: secondAsar,
        ...
        const firstManifestFile = path.join(firstOutput, "soulcord-build-manifest.json");
        const secondManifestFile = path.join(secondOutput, "soulcord-build-manifest.json");
        writeSoulCordPostBuildManifest(firstManifestFile, firstManifest);
        writeSoulCordPostBuildManifest(secondManifestFile, secondManifest);
    ...
    test("rejects dirty production builds while permitting explicitly diagnostic packaging", () => {
        ...
        expect(() => assertSoulCordBuildAllowed(production)).toThrow("clean Git worktree");
        expect(() => assertSoulCordPackagingAllowed(production, true)).toThrow("clean Git worktree");
        expect(() => assertSoulCordPackagingAllowed(diagnostic, false)).toThrow("production or release");
        expect(() => assertSoulCordPackagingAllowed(diagnostic, true)).not.toThrow();

        const mislabeled = {...diagnostic, mode: "development" as const, buildLabel: `development-dirty.${diagnostic.source.digest.slice(0, 16)}`};
        expect(() => assertSoulCordPackagingAllowed(mislabeled, true)).toThrow("explicitly diagnostic");
    });
    ...
    test("rejects source drift between build and packaging", () => {
        ...
        expect(() => assertSoulCordBuildStillCurrent(built, current)).toThrow("changed after the build");
    });
    ...
    test("writes validated embedded metadata and binds final artifacts without absolute paths", () => {
        ...
        expect(() => writeSoulCordBuildProvenance(metadataFile, {...provenance, localAbsolutePath: testFixture.root} as typeof provenance))
            .toThrow("Invalid SoulCord build provenance");
        writeSoulCordBuildProvenance(metadataFile, provenance);
        expect(readSoulCordBuildProvenance(metadataFile)).toEqual(provenance);

        const asarFile = path.join(testFixture.root, "soulcord.asar");
        ...
        fs.writeFileSync(packageFile, "{\"name\":\"soulcord\"}\n");
        fs.writeFileSync(checksums, `${crypto.createHash("sha256").update("final-asar").digest("hex")}  soulcord.asar\n`);
        const manifest = createSoulCordPostBuildManifest(provenance, {
            asar: asarFile,
        ...
        expect(manifest.artifacts.asar.file).toBe("soulcord.asar");
        expect(manifest.artifacts.packageMetadata.file).toBe("dist-package.json");
        expect(manifest.build.source.commit).toHaveLength(40);
        expect(JSON.stringify(manifest)).not.toContain("soulcord-provenance-test-");
    });
});
