...
import {
    ...
    SoulCordStreamAudienceGuard,
    SOULCORD_AUDIENCE_GUARD_PROMISE,
    type SoulCordAudienceGuardAdapter,
    type SoulCordAudienceGuardStatus
} from "../../src/betterdiscord/modules/soulcord/stream-audience-guard";

class FakeAudienceAdapter implements SoulCordAudienceGuardAdapter {
    accountId = "100";
    channelId = "200";
...
describe("SoulCord Stream Audience Guard", () => {
    ...
    test("starts ready and requires explicit per-call arming", () => {
        ...
        const statuses: SoulCordAudienceGuardStatus[] = [];
        const guard = new SoulCordStreamAudienceGuard(adapter, status => statuses.push(status));
        ...
        expect(guard.snapshot()).toEqual(expect.objectContaining({phase: "armed", armed: true, accountBound: true, channelBound: true, denylistCount: 1, detail: SOULCORD_AUDIENCE_GUARD_PROMISE}));
        expect(statuses.length).toBeGreaterThan(1);
    });

    test("prevents Go Live only when armed and a denied member is present", () => {
        const adapter = new FakeAudienceAdapter();
        const guard = new SoulCordStreamAudienceGuard(adapter);
        guard.start();
    ...
    test("fails unavailable and disposes every owned adapter resource", () => {
        ...
        const malformed = {...adapter, subscribe: undefined} as unknown as SoulCordAudienceGuardAdapter;
        const unavailable = new SoulCordStreamAudienceGuard(malformed);
