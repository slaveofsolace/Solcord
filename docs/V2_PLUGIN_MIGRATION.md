...
Solcord V2 replaces a reviewed community plugin only after its built-in adapter starts, passes its health check, and can be rolled back. A matching catalog card or source hash alone is not parity evidence.

## Transaction

1. Preview the exact community filenames, hashes, enabled states, replacement modules, dependencies, and archive destination.
2. Start and validate the selected Solcord built-ins without touching the community files.
...
Any source mismatch, ambiguous filename, persistence failure, adapter failure, linked path, or incomplete cleanup aborts or rolls back the transaction. Solcord never deletes a provider file. A file changed after preview is preserved for manual review.

Provider retirement has two distinct readiness checks. The renderer first starts
and health-checks the matching Solcord adapter. The main process then confines
the archive operation to replacement contracts compiled into that Solcord
...
- BDFDB and DevilBro community files remain separate GPL-2.0 programs and are not embedded in the Apache-2.0 Solcord core.
