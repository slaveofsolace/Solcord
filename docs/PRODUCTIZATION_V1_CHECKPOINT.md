...
- SoulCord no longer renders a competing launch layer; Discord's native spinning-logo splash remains unobstructed, and the inherited loading adapter is a side-effect-free no-op.
...
| --- | --- |
| `SOURCE_GREEN` | Granted for the current dirty-source snapshot: 599 tests, 3,500 assertions, zero failures; typecheck, SoulCord CSS, type generation, production audit, and reproducible diagnostic packaging pass. ESLint retains three inherited warnings and zero errors. Clean production packaging remains owner-commit-gated. |
...
- The native external-link review modal, DM navigation, exact Windows DPI changes, and live-profile replacement remain unclaimed. The isolated launcher visibly retained Discord's native startup spinner; no SoulCord overlay is intended.
