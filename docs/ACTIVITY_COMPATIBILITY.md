...
- SoulCord bridge exposure requires an exact approved Discord hostname, HTTPS with the default port, and a top-level frame. Rejected contexts still chain the matching Discord preload.
- SoulCord renderer injection is guarded per preload-document generation: duplicate calls in one document are ignored, while a full navigation in the same `webContents` receives one fresh injection. Execution targets the exact sending `WebFrameMain`; a detached or replaced frame fails closed.
...
- Disposable acceptance mode does not register `betterdiscord://`, confines SoulCord state to its declared absolute root, and disables updater/addon writes.
...
| BrowserWindow property policy unit | Rejects all later assignments by default | Plain-object descriptor fixture installs SoulCord, accepts one same-package assignment, and rejects an external assignment | Automated pass; Electron constructor integration pending disposable runtime |
