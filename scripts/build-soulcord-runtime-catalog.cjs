...
function main() {
    ...
    const catalogIndex = catalog.candidates.map(candidate => ({
        ...
        securityDisposition: reviewedByCatalogId.has(candidate.catalogId)
            ? securityDisposition(security, "candidates", reviewedByCatalogId.get(candidate.catalogId))
            : candidate.targetDisposition,
        requestedByPreset: candidate.requestedByPreset === true,
        immutableRevision: candidate.immutableRevision,
        licenseStatus: candidate.license?.status || "UNRESOLVED",
        codeStatus: candidate.verification?.code || "PENDING",
        runtimeStatus: candidate.verification?.runtime || "PENDING"
    }));
