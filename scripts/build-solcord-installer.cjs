...
if (!artifactInput || !outputInput || !/^[0-9a-f]{40}$/.test(sourceCommit ?? "")) {
    throw new Error("Usage: bun scripts/build-solcord-installer.cjs <betterdiscord.asar> <new-output-directory> <40-char-source-commit>");
}
...
if (fs.existsSync(dist)) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink() || path.dirname(dist) !== repo) throw new Error("The Solcord dist path is unsafe.");
...
if (!fs.existsSync(artifact) || !fs.lstatSync(artifact).isFile() || fs.lstatSync(artifact).isSymbolicLink()) throw new Error("The freshly built Solcord ASAR is missing or linked.");
const sourceBuildManifest = path.join(repo, "dist", "solcord-build-manifest.json");
if (!fs.existsSync(sourceBuildManifest) || !fs.lstatSync(sourceBuildManifest).isFile() || fs.lstatSync(sourceBuildManifest).isSymbolicLink() || fs.statSync(sourceBuildManifest).size > 256 * 1024) throw new Error("The authoritative Solcord build manifest is missing or unsafe.");
...
const bundledArtifact = path.join(output, "solcord.asar");
fs.copyFileSync(artifact, bundledArtifact, fs.constants.COPYFILE_EXCL);
const bundledBuildManifest = path.join(output, "solcord-build-manifest.json");
...
const manifest = {
    ...
    releaseNotes: "Unsigned Solcord V2 release candidate. Explicit install, verify, repair/update, rollback/uninstall, and launch only. Windows may display an unknown-publisher warning."
};
fs.writeFileSync(path.join(output, "solcord-installer-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, {encoding: "utf8", flag: "wx"});

const executable = path.join(output, "SolcordInstaller.exe");
