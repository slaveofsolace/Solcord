...
import type {SoulCordRelationshipChangeTransition, SoulCordRelationshipEvent} from "@common/soulcord/friend-watch";
...
function normalizeEvent(value: unknown): SoulCordRelationshipEvent | undefined {
    if (!value || typeof value !== "object") return;
    const event = value as Record<string, unknown>;
    if (event.schemaVersion !== 1 || typeof event.eventId !== "string" || !/^[a-zA-Z0-9_-]{8,96}$/.test(event.eventId)) return;
    const transition = TRANSITIONS.has(String(event.transition)) ? event.transition as SoulCordRelationshipEvent["transition"] : undefined;
    ...
    const normalizedBase = {
        ...
        source: event.source as SoulCordRelationshipEvent["source"],
        confidence: event.confidence as SoulCordRelationshipEvent["confidence"],
        schemaVersion: 1 as const,
        ...(typeof event.displayLabel === "string" && event.displayLabel.trim() ? {displayLabel: event.displayLabel.trim().slice(0, 160)} : {})
    };
    const normalized: SoulCordRelationshipEvent = transition === "reconciled"
        ? {...normalizedBase, transition}
        : subjectId
            ? {...normalizedBase, transition: transition as SoulCordRelationshipChangeTransition, subjectId}
            : {...normalizedBase, transition: transition as SoulCordRelationshipChangeTransition, subjectKey: subjectKey!};
    return Buffer.byteLength(JSON.stringify(normalized), "utf8") <= 2_048 ? normalized : undefined;
}

function normalizeRequest(value: unknown, operation: "append" | "read" | "clear"): {events: SoulCordRelationshipEvent[]; retentionDays: 7 | 30 | 90;} {
...
function prune(events: readonly SoulCordRelationshipEvent[], retentionDays: 7 | 30 | 90, now = Date.now()): SoulCordRelationshipEvent[] {
    const cutoff = now - retentionDays * 24 * 60 * 60 * 1_000;
    const unique = new Map<string, SoulCordRelationshipEvent>();
...
export class SoulCordFriendWatchStorage {
    #identityKey?: Buffer;
    #session = new Map<string, SoulCordRelationshipEvent[]>();
    ...
    append(rawAccount: unknown, rawRequest: unknown): Promise<{events: SoulCordRelationshipEvent[]; persistent: boolean;}> {
    ...
    read(rawAccount: unknown, rawRequest: unknown): Promise<{events: SoulCordRelationshipEvent[]; persistent: boolean; complete: boolean;}> {
        const account = this.#account(rawAccount);
    ...
    #appendSession(account: string, request: {events: SoulCordRelationshipEvent[]; retentionDays: 7 | 30 | 90;}): SoulCordRelationshipEvent[] {
        const events = prune([...(this.#session.get(account) ?? []), ...request.events], request.retentionDays);
        this.#session.set(account, events);
        return events;
    }

    #persistedEvent(account: string, event: SoulCordRelationshipEvent): SoulCordRelationshipEvent {
    ...
    #root(create: boolean): string | undefined {
        const base = path.resolve(resolveSoulCordBetterDiscordRoot(app.getPath("userData")));
        const root = path.join(base, "soulcord-friend-watch-v1");
    ...
    #readFile(directory: string, key: Buffer): SoulCordRelationshipEvent[] {
    ...
    #readEnvelopeFile(file: string, key: Buffer): SoulCordRelationshipEvent[] {
    ...
    #writeFile(directory: string, key: Buffer, events: SoulCordRelationshipEvent[]): void {
        this.#recoverAtomicFile(directory, key);
    ...
    #accountAad(directory: string): Buffer {
        ...
        return Buffer.from(`soulcord-friend-watch-v2\0${store}`, "utf8");
    }
...
export default new SoulCordFriendWatchStorage();
