...
export interface SoulCordReturnLaterItem {
    id: string;
    route: string;
    label: string;
    createdAt: number;
...
export function normalizeSoulCordReturnRoute(input: string): string | undefined {
    let url: URL;
...
function normalizeItem(value: unknown, now: number): SoulCordReturnLaterItem | undefined {
    if (!value || typeof value !== "object") return;
    const item = value as Record<string, unknown>;
    const route = normalizeSoulCordReturnRoute(typeof item.route === "string" ? item.route : "");
...
export class SoulCordReturnLaterJournal {
    #items: SoulCordReturnLaterItem[] = [];

    constructor(value?: unknown, now = Date.now()) {
        if (!Array.isArray(value)) return;
        const unique = new Map<string, SoulCordReturnLaterItem>();
    ...
    add(id: string, input: string, label: string, dueAt: number, now = Date.now()): SoulCordReturnLaterItem | undefined {
        const route = normalizeSoulCordReturnRoute(input);
    ...
    snapshot(includeCompleted = false): SoulCordReturnLaterItem[] {
