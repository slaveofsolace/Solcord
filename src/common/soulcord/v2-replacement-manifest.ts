...
export type SoulCordV2ReplacementTarget =
...
export interface SoulCordV2ReplacementEntry {
    cardName: string;
    fileName: string;
    dependencies: readonly string[];
    replacement: SoulCordV2ReplacementTarget;
...
export interface SoulCordV2ReplacementManifest {
    version: 2;
    entries: ReadonlyArray<Readonly<SoulCordV2ReplacementEntry>>;
}
...
function entry(
    ...
    dependencies: readonly string[],
    replacement: SoulCordV2ReplacementTarget,
    implementation: SoulCordV2ReplacementEntry["implementation"],
    privateData: SoulCordV2ReplacementEntry["privateData"] = "not-applicable"
): Readonly<SoulCordV2ReplacementEntry> {
...
const ENTRIES: ReadonlyArray<Readonly<SoulCordV2ReplacementEntry>> = [
...
export const SOULCORD_V2_REPLACEMENT_MANIFEST: SoulCordV2ReplacementManifest = Object.freeze({version: 2 as const, entries: Object.freeze(ENTRIES)});

export interface SoulCordV2RetirementInput {
    presentFiles: readonly string[];
    replacementReadyFiles: readonly string[];
    retainedBdfdbConsumers?: readonly string[];
}

export interface SoulCordV2RetirementStep {
    order: number;
    fileName: string;
    cardName: string;
    replacement: SoulCordV2ReplacementTarget;
...
export interface SoulCordV2RetirementBlocker {
    fileName: string;
    reason: "replacement-not-ready" | "bdfdb-consumer-not-retired" | "unreviewed-bdfdb-consumer";
    consumers?: readonly string[];
}

export interface SoulCordV2RetirementPlan {
    version: 2;
    steps: ReadonlyArray<Readonly<SoulCordV2RetirementStep>>;
    blockers: ReadonlyArray<Readonly<SoulCordV2RetirementBlocker>>;
}
...
export function findSoulCordV2Replacement(fileName: string): Readonly<SoulCordV2ReplacementEntry> | undefined {
    if (typeof fileName !== "string") return;
    const normalized = fileName.toLowerCase();
    return SOULCORD_V2_REPLACEMENT_MANIFEST.entries.find(candidate => candidate.fileName.toLowerCase() === normalized);
}

export function planSoulCordV2ProviderRetirement(input: SoulCordV2RetirementInput): SoulCordV2RetirementPlan {
    ...
    const dependency = findSoulCordV2Replacement("0BDFDB.plugin.js")!;
    const steps: SoulCordV2RetirementStep[] = [];
    const blockers: SoulCordV2RetirementBlocker[] = [];

    const presentFeatures = SOULCORD_V2_REPLACEMENT_MANIFEST.entries
