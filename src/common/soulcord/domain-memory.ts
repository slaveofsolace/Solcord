...
export type SoulCordDomainDecision = "allow" | "warn" | "block";

export interface SoulCordDomainMemoryRecord {
    protocol: "http:" | "https:";
    host: string;
    decision: SoulCordDomainDecision;
    createdAt: number;
    expiresAt: number;
}

export interface SoulCordDomainRisk {
    protocol?: "http:" | "https:";
    host?: string;
    restricted: boolean;
    reasons: string[];
...
export function inspectSoulCordDomain(input: string): SoulCordDomainRisk {
    let url: URL;
...
function normalizeRecord(value: unknown, now: number): SoulCordDomainMemoryRecord | undefined {
...
export class SoulCordDomainMemory {
    #records = new Map<string, SoulCordDomainMemoryRecord>();
    ...
    remember(input: string, decision: SoulCordDomainDecision, ttlMs: number, now = Date.now()): SoulCordDomainMemoryRecord | undefined {
        const risk = inspectSoulCordDomain(input);
    ...
    decision(input: string, now = Date.now()): SoulCordDomainMemoryRecord | undefined {
        const risk = inspectSoulCordDomain(input);
    ...
    snapshot(now = Date.now()): SoulCordDomainMemoryRecord[] {
