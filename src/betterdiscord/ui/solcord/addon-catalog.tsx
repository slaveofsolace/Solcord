                ...
                <div className="solcord-curated-list">
                    {group.addons.map(presentation => {
                        ...
                        return <div className="solcord-curated-row" key={addon.name}>
                            ...
                            <label className="solcord-toggle"><input type="checkbox" checked={addon.enabled} disabled={!canManage || !addon.installed || busy === addon.name || (!addon.enabled && !addon.builtIn && !addon.installable)} onChange={event => void toggle(addon.name, event.currentTarget.checked)} /><span>{addon.enabled ? "On" : "Off"}</span></label>
                        </div>;
                    })}
                </div>
            </details>)}
