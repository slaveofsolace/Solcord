...
function NativeSuitePanel({scope}: {scope: NativeSuiteScope;}) {
    ...
    const peopleSnapshot = controller?.peopleSnapshot();
    ...
    const saveVoiceFile = () => {
        ...
        try {requireController().saveReviewedVoiceNoteFile(); setVoiceRecording(false); setVoicePreview(undefined); setActionStatus("Voice note saved locally. Attach it through Discord yourself when ready.");}
    ...
    return <Section title={sectionTitle} summary={sectionSummary}>
        ...
        {scope !== "status" && <>
            ...
            {scope === "chat" && available("composer-toolkit") && <details><summary>Composer Toolkit</summary><div className="solcord-composer-lab">
                ...
                <textarea value={composerDraft} maxLength={64000} placeholder="Review a draft locally before sending" onChange={event => setComposerDraft(event.currentTarget.value)} /><div className="solcord-actions"><ActionButton onClick={reviewComposer}>Review draft</ActionButton></div>{composerProof && <div className="solcord-native-preview"><p><strong>{composerProof.characterCount.toLocaleString()} characters</strong><span>{composerProof.partCount} guarded part(s)</span></p>{composerProof.warnings.length ? composerProof.warnings.map(warning => <p key={warning}>{warning}</p>) : <p>No broad-mention, length, or unclosed-code-block warnings found.</p>}</div>}
    ...
    return <>
        <Section title="Theme and layout" summary="Theme the whole Discord shell, not only this panel.">
            <div className="solcord-appearance-controls">
                ...
                <label>Optional effect<select value={preferences.nativeSuite.motion.effect} onChange={event => updateMotionEffect({effect: event.currentTarget.value as typeof preferences.nativeSuite.motion.effect})}><option value="off">Off</option><option value="signal">Click signal</option><option value="snow">Snow</option><option value="rain">Rain</option><option value="stars">Shooting stars</option></select></label>
                ...
                {preferences.nativeSuite.motion.effect === "stars" && <label>Star angle<input type="number" min="-75" max="75" value={preferences.nativeSuite.motion.starAngleDegrees} onChange={event => updateMotionEffect({starAngleDegrees: Number(event.currentTarget.value)})} /></label>}
