...
import {
    ...
    SOULCORD_VOICE_NOTE_MAX_DURATION_MS,
    type SoulCordCallSnapshot,
    type SoulCordGlanceMessage,
    type SoulCordTranslationProvider,
    type SoulCordV2FeatureId,
    type SoulCordVoiceHealthSample
} from "@common/soulcord/v2-feature-models";
import {soulCordNativeSuiteFeatureForAddon, type SoulCordNativeSuiteFeature} from "@common/soulcord/builtin-addons";

import {SoulCordDisposalScope} from "./disposal";


export type SoulCordNativeSuiteMaturity = "ready" | "unavailable";

export interface SoulCordNativeSuiteStatus {
    id: SoulCordNativeSuiteFeature | "permission-lens" | "voice-health" | "local-identity-notes";
    title: string;
    maturity: SoulCordNativeSuiteMaturity;
    detail: string;
    enabledProviders: string[];
}

export interface SoulCordNativeSuiteAdapter {
    currentCall?(): SoulCordCallSnapshot | undefined;
    subscribeCall?(listener: () => void): () => void;
    setLocalVolume?(userId: string, percent: number): void;
    loadedChannelMessages?(channelId: string): SoulCordGlanceMessage[] | undefined;
    notificationIds?(scope: "guild" | "mentions" | "all"): string[];
    markNotificationsRead?(scope: "guild" | "mentions" | "all", ids: readonly string[]): void;
    voiceHealthSample?(): SoulCordVoiceHealthSample | undefined;
...
const TITLES: Readonly<Record<SoulCordNativeSuiteStatus["id"], string>> = Object.freeze({
...
function enabledByFeature(addons: Readonly<Record<string, boolean>>): Map<SoulCordNativeSuiteFeature, string[]> {
    const result = new Map<SoulCordNativeSuiteFeature, string[]>();
    for (const [name, enabled] of Object.entries(addons)) {
        if (!enabled) continue;
        const feature = soulCordNativeSuiteFeatureForAddon(name);
        if (!feature) continue;
...
function safeToken(value: string, maximumLength = 32): string {
    if (!new RegExp(`^[0-9A-Za-z._:-]{1,${maximumLength}}$`).test(value)) throw new Error("SoulCord action identifier is invalid.");
    return value;
}
...
export const SOULCORD_TRANSLATION_RESPONSE_MAX_BYTES = 1024 * 1024;

export async function readBoundedTranslationJson(response: Response): Promise<unknown> {
    const declaredSize = Number(response.headers.get("content-length"));
    if (Number.isFinite(declaredSize) && declaredSize > SOULCORD_TRANSLATION_RESPONSE_MAX_BYTES) {
    ...
    try {
        while (true) {
            ...
            if (size > SOULCORD_TRANSLATION_RESPONSE_MAX_BYTES) {
...
export class SoulCordNativeSuiteController {
    readonly #scope: SoulCordDisposalScope;
    readonly #adapter: SoulCordNativeSuiteAdapter;
    readonly #enabled: Map<SoulCordNativeSuiteFeature, string[]>;
    readonly #status = new Map<SoulCordNativeSuiteStatus["id"], SoulCordNativeSuiteStatus>();
    #composer?: SoulCordComposerToolkitController;
    #call?: SoulCordCallContextController;
    #audio?: SoulCordAudioConsoleController;
    #voiceNote?: SoulCordVoiceNoteStudioController;
    #translation?: SoulCordTranslationDeskController;
    #people?: SoulCordPeopleSpacesController;
    #glance?: SoulCordChannelGlanceController;
    #notifications?: SoulCordNotificationReviewController;
    #motion?: SoulCordMotionStudioController;
    #permissions?: SoulCordPermissionLensController;
    #identityNotes?: SoulCordLocalIdentityNotesController;
    #voiceHealth?: SoulCordVoiceHealthController;
    ...
    constructor(scope: SoulCordDisposalScope, addons: Readonly<Record<string, boolean>>, adapter: SoulCordNativeSuiteAdapter) {
        this.#scope = scope;
    ...
    start(): void {
        if (this.#disposed) throw new Error("SoulCord native suite is disposed.");
        this.#startComposer();
        ...
        this.#permissions = new SoulCordPermissionLensController();
        ...
        this.#identityNotes = new SoulCordLocalIdentityNotesController();
    ...
    statuses(): SoulCordNativeSuiteStatus[] {
    ...
    previewTranslation(provider: SoulCordTranslationProvider, endpoint: string | undefined, source: string, target: string, text: string) {
    ...
    async beginVoiceNoteFromUserGesture(): Promise<{recordingId: string;}> {
        ...
        try {
            ...
            recorder.addEventListener("dataavailable", event => {
                ...
                if (sizeBytes > SOULCORD_VOICE_NOTE_MAX_BYTES) {
            ...
            recorder.start(250);
            recording.limitTimer = globalThis.setTimeout(() => this.#abortVoiceNoteRecording(recording!, new Error("Voice-note recording reached the ten minute limit.")), SOULCORD_VOICE_NOTE_MAX_DURATION_MS);
        ...
        return new Promise((resolve, reject) => {
            const finish = () => {
                ...
                try {
                    ...
                    if (blob.size > SOULCORD_VOICE_NOTE_MAX_BYTES) throw new Error("Voice-note recording exceeded the 25 MiB limit.");
                    recording.blob = blob;
    ...
    prepareReviewedVoiceNoteUpload(channelId: string): void {
        ...
        const file = new File([recording.blob], `SoulCord-voice-note-${Date.now().toString(36)}.${extension}`, {type: intent.payload.mime});
    ...
    dispose(): void {
        ...
        for (const controller of [this.#composer, this.#call, this.#audio, this.#voiceNote, this.#translation, this.#people, this.#glance, this.#notifications, this.#motion, this.#permissions, this.#identityNotes, this.#voiceHealth]) {
            ...
            catch {/* the owning SoulCord scope still disposes every external resource */}
        }
        this.#status.clear();
    }
    ...
    #startComposer(): void {
        ...
        this.#composer = new SoulCordComposerToolkitController();
        const sync = () => {
            ...
            for (const editor of editors) {
                ...
                if (!host) continue;
                let counter = host.querySelector<HTMLElement>("[data-soulcord-composer-count]");
                if (!counter) {
                    counter = document.createElement("span");
                    counter.dataset.soulcordComposerCount = "true";
                    counter.className = "soulcord-composer-count";
            ...
            for (const element of document.querySelectorAll<HTMLElement>("time[datetime]:not([data-soulcord-complete-time])")) {
                const date = new Date(element.getAttribute("datetime") ?? "");
                if (!Number.isNaN(date.valueOf())) {
                    element.dataset.soulcordCompleteTime = "true";
        ...
        this.#scope.own(() => document.querySelectorAll("[data-soulcord-composer-count]").forEach(element => element.remove()), "element");
        sync();
    ...
    #startCallContext(): void {
        ...
        this.#call = new SoulCordCallContextController();
        const sync = () => {
    ...
    #renderCallBadge(connected: boolean): void {
        document.querySelector("[data-soulcord-call-badge]")?.remove();
        if (!connected) return;
        ...
        badge.dataset.soulcordCallBadge = "true";
        badge.className = "soulcord-call-badge";
    ...
    #startAudioConsole(): void {
        ...
        this.#audio = new SoulCordAudioConsoleController();
    ...
    #startVoiceNoteStudio(): void {
        ...
        this.#voiceNote = new SoulCordVoiceNoteStudioController();
    ...
    #startTranslationDesk(): void {
        ...
        this.#translation = new SoulCordTranslationDeskController();
    ...
    #startPeopleAndSpaces(): void {
        ...
        this.#people = new SoulCordPeopleSpacesController();
    ...
    #startChannelGlance(): void {
        ...
        this.#glance = new SoulCordChannelGlanceController();
    ...
    #startNotificationReview(): void {
        ...
        this.#notifications = new SoulCordNotificationReviewController();
    ...
    #startMotionStudio(): void {
        ...
        this.#motion = new SoulCordMotionStudioController();
        ...
        if (providers.length && !settings.reducedMotion) this.#scope.style("soulcord-native-motion", `:root{--soulcord-native-motion:${settings.durationMs}ms} #app-mount [role="dialog"],#app-mount [class*="menu_"]{animation:soulcord-native-enter var(--soulcord-native-motion) ease-out}@keyframes soulcord-native-enter{from{opacity:.72;transform:translateY(3px)}to{opacity:1;transform:none}}`);
    ...
    #startVoiceHealth(): void {
        this.#voiceHealth = new SoulCordVoiceHealthController();
    ...
    #startFocusChannels(): void {
        ...
        this.#scope.own(() => document.querySelectorAll("[data-soulcord-focus-muted]").forEach(element => element.removeAttribute("data-soulcord-focus-muted")), "element");
        this.#scope.own(() => document.querySelector("[data-soulcord-call-badge]")?.remove(), "element");
        apply();
    }
    ...
    #applyFocusChannels(rawIds: readonly string[]): void {
        ...
        for (const item of document.querySelectorAll<HTMLElement>("[data-list-item-id*='channels___'], [data-list-item-id*='channel_']")) {
            ...
            if (!ids.size || !match || ids.has(match[1])) item.removeAttribute("data-soulcord-focus-muted");
            else item.dataset.soulcordFocusMuted = "true";
        }
    }

    #setStatus(id: SoulCordNativeSuiteStatus["id"], maturity: SoulCordNativeSuiteMaturity, detail: string, providers: string[]): void {
        this.#status.set(id, {id, title: TITLES[id], maturity, detail, enabledProviders: [...providers]});
    }
}

export const SOULCORD_NATIVE_SUITE_FEATURE_IDS: readonly SoulCordV2FeatureId[] = Object.freeze([
