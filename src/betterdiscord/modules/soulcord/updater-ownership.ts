...
export type SoulCordManagedKind = "plugin" | "theme";

export interface SoulCordUpdateOwnershipQuery {
    accepted: boolean;
    addonFolder: string;
    fileName: string;
    kind: SoulCordManagedKind;
...
function safeFileName(value: string, kind: SoulCordManagedKind): boolean {
...
/**
 * Returns true only for exact accepted bytes that SoulCord itself added in a
 * completed, unrolled transaction. Reused owner files are deliberately not
 * treated as SoulCord-owned, even when their bytes match the reviewed hash.
 */
export function isSoulCordTransactionOwnedAcceptedArtifact(query: SoulCordUpdateOwnershipQuery): boolean {
