export const SOULCORD_AUDIENCE_GUARD_PROMISE = "Your stream will not start or continue while a denied user is detected in the current call or viewer list.";

const DISCORD_ID = /^\d{1,32}$/;
const MAX_DENIED_USERS = 100;
const STOP_VERIFICATION_MS = 3_000;

export interface SoulCordAudienceGuardModes {
    preventStart: boolean;
    stopOnJoin: boolean;
    stopOnWatch: boolean;
}

export interface SoulCordAudienceGuardEntry {
    userId: string;
    label?: string;
}

export interface SoulCordAudienceGuardPrivatePolicy {
    version: 1;
    entries: SoulCordAudienceGuardEntry[];
}

export type SoulCordAudienceGuardPhase = "off" | "ready" | "armed" | "blocked" | "stopping" | "attention" | "unavailable";

export interface SoulCordAudienceGuardStatus {
    phase: SoulCordAudienceGuardPhase;
    detail: string;
    available: boolean;
    armed: boolean;
    ...
    activeModes: SoulCordAudienceGuardModes;
    lastTrigger?: "prevent-start" | "stop-on-join" | "stop-on-watch";
}

export interface SoulCordAudienceGuardAdapter {
...
export function normalizeAudienceGuardEntries(value: unknown): SoulCordAudienceGuardEntry[] {
    if (!Array.isArray(value)) return [];
    const entries: SoulCordAudienceGuardEntry[] = [];
...
export function normalizeAudienceGuardPrivatePolicy(value: unknown): SoulCordAudienceGuardPrivatePolicy {
...
export function validateAudienceGuardAdapter(adapter: SoulCordAudienceGuardAdapter): boolean {
...
const NO_MODES: SoulCordAudienceGuardModes = {preventStart: false, stopOnJoin: false, stopOnWatch: false};

export class SoulCordStreamAudienceGuard {
    #adapter: SoulCordAudienceGuardAdapter;
    #onStatus?: (status: SoulCordAudienceGuardStatus) => void;
    #started = false;
    #available = false;
    #armedAccountId?: string;
    #armedChannelId?: string;
    #denied = new Set<string>();
    #modes: SoulCordAudienceGuardModes = {...NO_MODES};
    #unsubscribe?: () => void;
    #unpatchStart?: () => void;
    #verifyTimer?: unknown;
    #stopLatch?: unknown;
    #status: SoulCordAudienceGuardStatus = {
        phase: "off",
    ...
    constructor(adapter: SoulCordAudienceGuardAdapter, onStatus?: (status: SoulCordAudienceGuardStatus) => void) {
    ...
    arm(entries: unknown, modes: SoulCordAudienceGuardModes): boolean {
        ...
        this.#publish({
            phase: "armed",
            detail: SOULCORD_AUDIENCE_GUARD_PROMISE,
            available: true,
            armed: true,
            accountBound: true,
            channelBound: true,
    ...
    snapshot(): SoulCordAudienceGuardStatus {
    ...
    synchronize(): void {
        ...
        if (!stream) {
            this.#stopLatch = undefined;
            this.#clearVerification();
            if (this.#status.phase !== "armed") this.#publish({phase: "armed", detail: SOULCORD_AUDIENCE_GUARD_PROMISE, detectedCount: 0, lastTrigger: undefined});
            return;
        }
            ...
            void Promise.resolve(result).then(() => {
                ...
                this.#verifyTimer = this.#adapter.setTimer(() => {
                    ...
                    if (this.#adapter.currentStream()) {
                        this.#publish({phase: "attention", detail: "SoulCord could not verify that Go Live stopped. Stop sharing manually now.", detectedCount, lastTrigger: trigger});
                    }
                    else {
                        this.#stopLatch = undefined;
    ...
    #stopFailed(trigger: "stop-on-join" | "stop-on-watch", detectedCount: number): void {
        this.#publish({phase: "attention", detail: "SoulCord could not request a verified Go Live stop. Stop sharing manually now.", detectedCount, lastTrigger: trigger});
    }
    ...
    #publish(patch: Partial<SoulCordAudienceGuardStatus>): void {
